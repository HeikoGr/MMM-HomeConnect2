...
const checkResponseStatus = (res) => {
        ...
        return res.text().then((text) => {
            ...
            const truncated = typeof text === "string" && text.length > 1000
                ? `${text.slice(
                    0,
                    1000
                )}... (truncated)`
    ...
    getClient = function getClient (accessToken) {
        ...
        return Promise.resolve({
            ...
            "apis": {
                "appliances": {
                    "get_home_appliances": () => makeApiRequest(
                        "GET",
                        "api/homeappliances",
                        accessToken
                    )
                ...
                "status": {
                    "get_status": (params) => makeApiRequest(
                        "GET",
                ...
                "settings": {
                    "get_settings": (params) => makeApiRequest(
                        "GET",
    ...
    refreshToken = (clientSecret, refreshTokenValue) => new Promise((resolve, reject) => {

        if (!fetch) {

            reject(new Error("Global fetch is not available in this Node runtime"));
            return;
        ...
        fetch(
            `${global.isSimulated
                ? global.urls.simulation.base
                : global.urls.physical.base}security/oauth/token`,
            {
                "method": "POST",
                "headers": {"content-type": "application/x-www-form-urlencoded"},
                "body": `grant_type=refresh_token&client_secret=${clientSecret}&refresh_token=${refreshTokenValue}`
            }
        ).
            then(checkResponseStatus).
            then((res) => res.json()).
...
class HomeConnect extends EventEmitter {
    ...
    async init (options) {
        ...
        global.isSimulated = typeof options !== "undefined" && "isSimulated" in options && typeof options.isSimulated === "boolean"
            ? options.isSimulated
            : false;
    ...
    async command (tag, operationId, haId, body) {
        ...
        return this.client.apis[tag][operationId]({haId,
            body});
    ...
    subscribeDevice (haid, event, callback) {
        ...
        this.eventSources[haid].addEventListener(
            event,
            callback
        );
        this.eventListeners[haid].set(
            event,
            callback
        );
    ...
    recreateEventSources () {
        ...
        for (const haid of Object.keys(this.eventSources)) {
            ...
            for (const [
                event,
                callback
            ] of this.eventListeners[haid]) {

                this.eventSources[haid].removeEventListener(
                    event,
                    callback
                );
