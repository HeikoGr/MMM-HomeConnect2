...
const RATE_LIMIT_DEFER_MAX_MS = 24 * 60 * 60 * 1000; // re-check at least daily

// OAuth answers for a clientId the Home Connect server does not accept.
const INVALID_CLIENT_ERRORS = new Set(["invalid_client", "unauthorized_client"]);
...
module.exports = {
  ...
  // Starting a session costs a token refresh, a full snapshot and the SSE
  // channels - during a block (e.g. restored after a restart) every one of them
  // would draw another 429. Wait until the block ends, then start once.
  deferWhileRateLimited(targetInstanceId = null) {
    if (!this.isRateLimited()) {
      return false;
    }
    const remainingMs = this.getRateLimitUntil() - Date.now();
    this.emitRateLimitNotice(targetInstanceId);
    if (!this.rateLimitDeferTimer) {
      log.info(`Rate limit active - starting the session in ${Math.ceil(remainingMs / 1000)}s`);
      // Capped to stay within setTimeout's range; a longer block defers again.
      this.rateLimitDeferTimer = setTimeout(
        () => {
          this.rateLimitDeferTimer = null;
          this.checkTokenAndInitialize();
        },
        Math.min(remainingMs + 1000, RATE_LIMIT_DEFER_MAX_MS),
      );
    }
    return true;
  },

  emitRateLimitNotice(targetInstanceId = null) {
    const seconds = Math.max(1, Math.ceil((this.getRateLimitUntil() - Date.now()) / 1000));
    this.emitInitStatus(
      "device_error",
      {
        message: `Rate limit active - wait ${seconds}s`,
  ...
  checkTokenAndInitialize(targetInstanceId = null) {
    if (this.deferWhileRateLimited(targetInstanceId)) {
      return;
    }
  ...
  handleHeadlessAuthError(error) {
    ...
    this.pendingAuthInfo = null;

    // lastAuthFailure is kept for displays that connect after the failure
    // (see handleConfigNotificationSubsequent).
    // Home Connect does not know this clientId - another attempt cannot change
    // that, only a corrected config.js can.
    if (INVALID_CLIENT_ERRORS.has(error.oauthError)) {
      log.error(
        `Home Connect rejected the clientId (${error.message}) - check clientId in config.js and restart MagicMirror`,
      );
      this.lastAuthFailure = { message: error.message, reason: "invalid_client" };
      this.emitAuthStatus("error", this.lastAuthFailure);
      return;
    }

    log.error(`Headless authentication failed: ${error.message}`);

    this.lastAuthFailure = { message: `Authentication failed: ${error.message}` };
    this.emitAuthStatus("error", this.lastAuthFailure);
  ...
  async initWithHeadlessAuth() {
    ...
    try {
      tokens = await this.authService.headlessAuth((notification, payload) => {
        ...
        if (notification === "AUTH_INFO") {
          // Displays that connect while the user is still logging in need the
          // QR code too (see notifyAuthInProgress).
          this.pendingAuthInfo = { payload, issuedAt: Date.now() };
        }
  ...
  handleHomeConnectInitError(error) {
    ...
    // A 429 from the token endpoint during init: the client reports it only from its own
    // refresh loop, so record the block here - otherwise the retry below would keep asking.
    if (error?.statusCode === 429) {
      const seconds = Number(error.retryAfterSeconds) > 0 ? Number(error.retryAfterSeconds) : 300;
      const until = Date.now() + seconds * 1000;
      if (until > this.getRateLimitUntil()) {
        this.setRateLimitUntil(until);
      }
      log.warn(`Rate limit during HomeConnect initialization - backing off ${seconds}s`);
    }
  ...
  scheduleHomeConnectInitRetry() {
    ...
    let delay = Math.min(HC_INIT_RETRY_BASE_DELAY_MS * 2 ** attempt, HC_INIT_RETRY_MAX_DELAY_MS);
    // Never retry into a rate-limit block; capped to stay within setTimeout's range.
    if (this.isRateLimited()) {
      delay = Math.min(Math.max(delay, this.getRateLimitUntil() - Date.now() + 1000), RATE_LIMIT_DEFER_MAX_MS);
    }
    this.hcInitRetryAttempts = attempt + 1;

    log.info(`Scheduling HomeConnect init retry in ${Math.round(delay / 1000)}s (attempt ${this.hcInitRetryAttempts})`);
    ...
    this.hcInitRetryTimer = setTimeout(() => {
      ...
      // A block set in the meantime (or one longer than the cap) is waited out first.
      if (this.isRateLimited()) {
  ...
  async initializeHomeConnect(refreshToken) {
    return new Promise((resolve, reject) => {
      ...
      const hc = new HomeConnect(this.config.clientId, this.config.clientSecret, refreshToken, {
        acceptLanguage: this.config.language,
        logger: log,
      });
      this.hc = hc;
