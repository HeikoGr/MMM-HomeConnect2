...
class ProgramFetchCoordinator {
  ...
  request(payload = {}) {
    ...
    // The block holds for forced requests too (SSE, follow-ups): during it every call can only
    // draw another 429. The scheduled snapshot catches up once it has ended.
    if (this.deps.isRateLimited()) {
      const remainingSeconds = Math.ceil((this.session.rateLimitUntil - now) / 1000);
      if (force) {
        log.info(`Rate limited - skipping forced active program fetch, ${remainingSeconds}s remaining`);
        return;
      }
      ...
      results.forEach((result) => {
        if (result.success && result.data) {
          ...
          if (payload) {
            ...
            // Running, yet only the selected program came back: right after the
            // start /programs/active can lag behind the state - ask again shortly.
            const device = this.deps.getDeviceService().devices.get(result.haId);
            if (result.source === "selected" && device && deviceAppearsActive(device)) {
              log.debug(`${device.name} runs but reported only its selected program - retrying`);
