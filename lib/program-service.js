...
class ProgramService {
  ...
  async fetchAvailableProgramDefinition(haId, programKey, deviceName) {
    ...
    this.logger.debug(`Fetching available program constraints for ${deviceName} (${programKey})`);
  ...
  async fetchAvailableProgramsSnapshot(haId, deviceName) {
    ...
    const firstProgram = programs.find((program) => program?.key);
  ...
  async enrichProgramResult(haId, deviceName, result) {
    ...
    result.availableProgram = await this.fetchAvailableProgramDefinition(haId, result.data.key, deviceName);
    return result;
  }
  ...
  applyProgramResult(result) {
    ...
    if (result.source === "available") {
      ...
      device.AvailableProgramName = pickProgramName(result.data.programDefinition) || availablePrograms[0] || null;
