...
class ProgramService {
  ...
  async fetchActiveProgramForDevice(haId, deviceName) {
    ...
    try {
      this.logger(
        "debug",
        `Fetching active program for ${deviceName} (${haId})`
      );
      if (this.hc && typeof this.hc.getActiveProgram === "function") {
        ...
        if (res.success) {
          ...
          this.logger(
            "debug",
            `Active program data received for ${deviceName} in ${durationMs}ms`
          );
          if (durationMs > LONG_ACTIVE_PROGRAM_REQUEST_MS) {
            this.logger(
              "warn",
              `Slow active program response for ${deviceName}: ${durationMs}ms`
            );
          }
      ...
      const err = new Error(
        "HomeConnect client missing getActiveProgram wrapper"
      );
  ...
  broadcastProgramData(
    ...
    this.logger(
      "info",
      `Active programs fetched: ${deviceNames.length} with data`
    );
  ...
  handleActiveProgramFetchError(error, broadcastToAllClients) {
    ...
    if (
      ...
      const backoffMinutes = Math.min(
        2 * Math.pow(2, Math.floor(Math.random() * 3)),
        10
      );
