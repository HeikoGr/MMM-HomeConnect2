...
module.exports = {
  ...
  // Both are needed: hideConfigSecrets makes the browser send the redacted values
  // (e.g. clientSecret "***"), so isOutdatedClientConfig must be able to match
  // against the redacted copy too.
  serverConfigs() {
  ...
  notifyAuthInProgress(instanceId) {
    ...
    if (!pending) {
      ...
      log.debug("Session start in progress - display will be updated when it completes", { instanceId });
      this.emitInitStatus("initializing", { instanceId }, { broadcast: false, targetInstanceId: instanceId });
      return;
    }
  ...
  handleConfigNotificationSubsequent(instanceId) {
    if (this.sessionAuthenticated && this.hc && this.deviceService) {
      ...
      this.emitRateLimitNotice(instanceId);
  ...
  handleConfigNotification(payload) {
    ...
    if (!this.configReceived) {
      ...
      // The browser payload may contain redacted values (e.g. clientSecret "***")
      // when hideConfigSecrets is on. Override with the full server config so that
      // AuthService always receives real credentials for backend authentication.
