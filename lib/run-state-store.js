...
// A dryer stays in Run through its wrinkle guard (up to two hours) with remaining
// time 0 and progress 100: the program itself is over. The first sighting of that is
// the end of the run - and only then a real end time when the run was watched up to
// it (programEndObserved), not merely found in that state after a restart.
function notePastProgramEnd(record, device, state, now, watched) {
  if (!record || Number.isFinite(record.programEndedAt) || !state.hasProgramInProgress) {
    return false;
  }
  if (record.programKey !== device.ActiveProgramKey) {
    return false;
  ...
  const progress = Number(device.ProgramProgress);
  const pastEnd = remainingSeconds === 0 || (!(remainingSeconds > 0) && Number.isFinite(progress) && progress >= 100);
  if (!pastEnd) {
    return false;
  }
  record.programEndedAt = now;
  record.programEndObserved = watched;
  return true;
}

// End time of a run: the program end noted above if there is one, else the moment
// the appliance left the run - either only when this process watched it happen.
function runEndedAt(record, endObserved, now) {
  if (Number.isFinite(record.programEndedAt)) {
    return record.programEndObserved === true ? record.programEndedAt : null;
  }
  return endObserved ? now : null;
}

// A run that went through an error state and did not finish counts as an error.
function endOutcome(state, record) {
  if (state.isFinished) {
    return "finished";
  }
  return record.sawError === true ? "error" : "aborted";
}
...
function reconcileRunStates(devices, records, now = Date.now(), session = {}) {
  ...
  const endRun = (haId, device, record, outcome, endObserved) => {
    // A run that reached its program end finished, whatever the appliance does after.
    const finalOutcome = Number.isFinite(record.programEndedAt) ? "finished" : outcome;
    ended.push({ haId, device, record, outcome: finalOutcome, endedAt: runEndedAt(record, endObserved, now) });
    delete records[haId];
    changed = true;
  };
  ...
  for (const device of devices) {
    ...
    // Only a state without a program (or one still waiting for its delayed start)
    // means a later run is watched from its start. An error state does not: the
    // appliance may resume the program it was running.
    if (state.hasNoProgram || state.isFinished || state.isDelayedStart) {
      seenIdle.add(haId);
    }

    // An error state interrupts a run without ending it; the record stays and
    // only remembers it, for the outcome once the run really ends.
    if (state.isError) {
      if (record && record.sawError !== true) {
        record.sawError = true;
        changed = true;
      }
      continue;
    }
    ...
    if (state.known && !state.hasProgramInProgress) {
      if (record) {
        endRun(haId, device, record, endOutcome(state, record), seenRunning.has(haId));
      }
      seenRunning.delete(haId);
      continue;
    }

    if (notePastProgramEnd(record, device, state, now, seenRunning.has(haId))) {
      changed = true;
      continue;
    }
