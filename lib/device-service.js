...
class DeviceService {
  ...
  noteDeviceFetchFailure(kind, device, result) {
    const statusCode = (result && result.statusCode) || null;
    const message = (result && result.error) || "unknown error";

    if (!isRateLimitError({ statusCode, message })) {
      this.logger(
        "warn",
        `${kind} fetch failed for ${device.name} (${statusCode || "n/a"}): ${message}`
      );
      return;
    }

    const retryAfterSeconds = Number.isFinite(result && result.retryAfterSeconds)
    ...
    this.logger(
      "warn",
      `Rate limit on ${kind} fetch for ${device.name} - backing off for ${retryAfterSeconds}s`
    );
  }
  ...
  processDevice(device, index) {
    return this.refreshDeviceDetails(this.registerDevice(device, index));
  }
