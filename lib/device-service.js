...
class DeviceService {
  ...
  fetchDeviceStatus(device, sendSocketNotification) {
    if (this.hc && typeof this.hc.getStatus === "function") {
      this.hc
        ...
        .catch((err) => this.logger("error", `Status error for ${device.name}:`, err));
      return;
    }
    this.logger(
      "error",
  ...
  fetchDeviceSettings(device, sendSocketNotification) {
    if (this.hc && typeof this.hc.getSettings === "function") {
      this.hc
        ...
        .catch((err) => this.logger("error", `Settings error for ${device.name}:`, err));
      return;
    }
    this.logger(
      "error",
  ...
  processDevice(device, index, sendSocketNotification) {
    ...
    if (connected) {
      ...
      this.fetchDeviceStatus(deviceRef, sendSocketNotification);
      this.fetchDeviceSettings(deviceRef, sendSocketNotification);
  ...
  subscribeToDeviceEvents(deviceEventHandler) {
    ...
    if (!this.subscribed) {
      ...
      this.logger("debug", "Already subscribed to device events - skipping duplicate subscription");
    }
  }
