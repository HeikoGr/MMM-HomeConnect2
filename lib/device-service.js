...
// these are no config options. Tests inject shorter timings (options.heartbeat).
const HEARTBEAT_DEFAULTS = Object.freeze({
  enabled: true,
  checkIntervalMs: 10 * 1000,
  staleThresholdMs: 70 * 1000,
  // Minimum time between two automatic rebuilds.
  recoveryCooldownMs: 70 * 1000,
});
...
class DeviceService {
  constructor(options) {
    ...
    this.heartbeatIntervalMs = heartbeat.checkIntervalMs;
    ...
    this.staleRecoveryCooldownMs = heartbeat.recoveryCooldownMs;
