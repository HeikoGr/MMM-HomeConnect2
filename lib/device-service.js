...
class DeviceService {
  constructor(options) {
    ...
    this.globalEventSubscribeDelayMs = 0;
  ...
  attachClient(hc) {
    ...
    this.hc = hc;
    this.applyEventSourceRetryConfig();
  }
  ...
  setConfig(config = {}) {
    ...
    this.globalEventSubscribeDelayMs = config.globalEventSubscribeDelayMs || 0;
    this.eventSourceRetryBaseDelayMs = config.eventSourceRetryBaseDelayMs || 5000;
    this.eventSourceAuthErrorDelayMs = config.eventSourceAuthErrorDelayMs || 30000;
  ...
  applyEventSourceRetryConfig() {
    if (this.hc && typeof this.hc.setEventSourceRetryConfig === "function") {
      this.hc.setEventSourceRetryConfig({
        baseDelayMs: this.eventSourceRetryBaseDelayMs,
        authDelayMs: this.eventSourceAuthErrorDelayMs
      });
    }
  }
  ...
  processDevice(device, index) {
    this.logger("debug", `Processing device ${index + 1}: ${device.name} (${device.haId})`);
    this.logger("debug", `Raw connected flag for ${device.name}:`, device.connected);
  ...
  scheduleGlobalSubscriptions() {
    if (this.subscribed || this.globalEventSubscribeDelayMs <= 0) {
    ...
    if (this.globalSubscribeTimer) {
      this.logger(
        "debug",
        "Global SSE subscription already scheduled - waiting for existing timer"
      );
      return;
    }
    ...
    this.logger(
      "info",
      `Delaying global SSE subscription by ${this.globalEventSubscribeDelayMs}ms per config`
    );
    this.globalSubscribeTimer = setTimeout(() => {
      this.globalSubscribeTimer = null;
      this.establishEventSubscriptions();
    }, this.globalEventSubscribeDelayMs);
  }
