...
class DeviceService {
  ...
  broadcastDevices(sendSocketNotification) {
    ...
    sendSocketNotification("MMM-HomeConnect_Update", devices);
  }
  ...
  startHeartbeatMonitor() {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      const referenceTimestamp = this.heartbeatArmed
        ? this.lastEventTimestamp
        : this.lastSubscriptionTimestamp;
      ...
      const now = Date.now();
      const silenceMs = now - referenceTimestamp;
      const waitingForFirstEvent = !this.heartbeatArmed;
      ...
      if (silenceMs >= this.heartbeatStaleThresholdMs && !this.heartbeatStale) {
        ...
        this.logger(
          ...
          waitingForFirstEvent
            ? `No SSE events received after subscription for ${durationLabel} - broadcasting stale status`
            : `No SSE events received for ${durationLabel} - broadcasting stale status`
        );
        this.broadcastToAllClients("INIT_STATUS", {
          status: "sse_stale",
          message: waitingForFirstEvent
            ? `No Home Connect events received after SSE subscription for ${durationLabel}`
            : `No Home Connect events received for ${durationLabel}`
        });
  ...
  handleGetDevicesError(error) {
    ...
    this.broadcastToAllClients("INIT_STATUS", {
      status: "device_error",
      message: isRateLimit ? `HTTP 429: ${message}` : `Device error: ${message}`,
  ...
  deviceEvent(data, sendSocketNotification) {
    try {
      ...
      if (processed) {
        ...
        this.broadcastDevices(sendSocketNotification);
        this.lastEventTimestamp = Date.now();
        this.lastSubscriptionTimestamp = this.lastEventTimestamp;
