  ...
  applyEventToDevice(device, event) {
    ...
    switch (key) {
        ...
        try {
          ...
          if (Number.isFinite(rem) && rem > 0) {
            ...
            if (staleCompletedProgress) {
              delete device.ProgramProgress;
              delete device["BSH.Common.Option.ProgramProgress"];
              delete device["BSH.Common.Status.ProgramProgress"];
            }
      ...
      case "BSH.Common.Status.OperationState": {
        ...
        // "Inactive"/"Ready" mean no program exists any more - typically reported
        // when the door is opened after a finished cycle. The appliance stops
        // sending progress at that point, so a leftover value (often a reset 0)
        // would survive for the lifetime of the process and paint a stale bar on
        // long-running instances, while a freshly started one shows nothing.
        ...
        if (stateValue === "BSH.Common.EnumType.OperationState.Finished") {
          ...
          delete device.RemainingProgramTimeIsEstimated;
        }
        break;
      }
