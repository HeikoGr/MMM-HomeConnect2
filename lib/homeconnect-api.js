...
class HomeConnect extends EventEmitter {
  ...
  subscribeDevice(haid, event, callback) {
    if (this.eventSources && !(haid in this.eventSources)) {
      const url = global.isSimulated ? global.urls.simulation.base : global.urls.physical.base,
        eventSource = new ESConstructor(`${url}api/homeappliances/${haid}/events`, {
          headers: {
    ...
    try {
      this.eventSources[haid].addEventListener("open", () => {
        console.debug && console.debug(`EventSource open for ${haid}`);
      });
      this.eventSources[haid].addEventListener("error", (err) => {
        console.error(`EventSource error for ${haid}:`, err && err.message ? err.message : err);
        // Avoid rapid recreate loops: only recreate if last error was >5s ago
        this._lastEventErrorAt = this._lastEventErrorAt || 0;
        if (Date.now() - this._lastEventErrorAt > 5000) {
          this._lastEventErrorAt = Date.now();
          try {
            this.recreateEventSources();
          } catch (e) {
            console.error("Failed to recreate event sources:", e);
          }
        }
      });
    } catch {
      // ignore if environment doesn't support addEventListener signature
    }
  ...
  subscribe(event, callback) {
    ...
    // Monitor EventSource connectivity
    try {
      ...
      this.eventSource.addEventListener("error", (err) => {
        console.error(
          "EventSource error for global events:",
        ...
        if (Date.now() - this._lastEventErrorAt > 5000) {
          ...
          try {
            ...
            console.error("Failed to recreate global event source:", e);
          }
        }
      });
    } catch {
  ...
  recreateEventSources() {
    for (const haid of Object.keys(this.eventSources)) {
      ...
      const url = global.isSimulated ? global.urls.simulation.base : global.urls.physical.base;
      this.eventSources[haid] = new ESConstructor(`${url}api/homeappliances/${haid}/events`, {
        headers: {
          accept: "text/event-stream",
          authorization: `Bearer ${this.tokens.access_token}`
        }
