...
class HomeConnect extends EventEmitter {
  ...
  recreateEventSources() {
    ...
    for (const haid of Object.keys(this.eventSources)) {
      ...
      if (existing && typeof existing.close === "function") {
        try {
          ...
          this.log.warn(`Failed to close stale EventSource for ${haid}:`, err?.message ? err.message : err);
        }
    ...
    const shouldRecreateGlobal = this.eventSource || (this.eventListener && this.eventListener.size > 0);
    if (shouldRecreateGlobal) {
      if (this.eventSource) {
        try {
          ...
          this.log.warn("Failed to close stale global EventSource:", err?.message ? err.message : err);
        }
