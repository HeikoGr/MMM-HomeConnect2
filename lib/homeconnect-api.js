...
// Reported by the first client instance through its logger: at load time no
// log level is configured yet.
let esLoadError = null;
if (!ESConstructor) {
  try {
    const es = require("eventsource");
    ESConstructor = es && (es.default || es.EventSource || es);
  } catch (error) {
    esLoadError = error.message;
  }
}
...
const DELAYED_START_STATE = "BSH.Common.EnumType.OperationState.DelayedStart";
const RUN_STATE = "BSH.Common.EnumType.OperationState.Run";

// First sighting of a positive remaining time: the start of the run as far as this
// process knows (_remainingObservedAt), and the duration it announced then. With a
// delayed start the program has not begun - the remaining time does not count down
// yet, and its first sighting is not the program start (see trackDelayedStart).
function noteRemainingSeen(device, remainingSeconds, now) {
  if (device.OperationState === DELAYED_START_STATE) {
    return;
  }
  if (!device._initialRemaining) {
    device._initialRemaining = remainingSeconds;
  }
...
// The program start is when a delayed start ends, not when its remaining time was
// first reported: waiting for the start and the run itself are kept apart (progress
// bar, run_state.json, program statistics).
function trackDelayedStart(device, previousState, now) {
  if (device.OperationState === DELAYED_START_STATE) {
    clearDeviceFields(device, OBSERVED_RUNTIME_KEYS);
    return;
  }
  if (previousState !== DELAYED_START_STATE || device.OperationState !== RUN_STATE) {
    return;
  }
  clearDeviceFields(device, OBSERVED_RUNTIME_KEYS);
  const remainingSeconds = parseDurationSeconds(device.RemainingProgramTime);
  if (Number.isFinite(remainingSeconds) && remainingSeconds > 0) {
    noteRemainingSeen(device, remainingSeconds, now);
  }
}

const logToConsole = Object.freeze({
...
// Home Connect base URLs. Module-local on purpose: Node's `global` is shared by every
// MagicMirror module in the process.
const BASE_URLS = {
  simulation: "https://simulator.home-connect.com/",
  physical: "https://api.home-connect.com/",
};

// Set by HomeConnect#init(); the module always runs against the physical API.
let isSimulated = false;
const apiBaseUrl = () => (isSimulated ? BASE_URLS.simulation : BASE_URLS.physical);
...
function createHttpError(res, responseText) {
  ...
  err.retryAfterSeconds = parseRetryAfterSeconds(res?.headers);
  return err;
}
...
const fetchWithTimeout = (url, options = {}, requestOptions = {}) => {
  ...
  return fetch(url, {
    ...
    .catch((error) => {
      const aborted = controller.signal.aborted || error?.name === "AbortError" || error?.code === "ABORT_ERR";
...
function ensureSummaryBucket(device, bucketKey) {
  if (!device[bucketKey] || typeof device[bucketKey] !== "object" || Array.isArray(device[bucketKey])) {
    device[bucketKey] = {};
  }
...
function applyRefrigerationState(device, event) {
  ...
  if (/Door/i.test(key)) {
    ...
    if (doorState) {
      setSummaryEntry(device, "RefrigerationDoorStates", compartment, `${compartment}: ${doorState}`);
      setSummaryEntry(device, "DeviceStatusByKey", `refrigeration-door-${compartment}`, `${compartment}: ${doorState}`);
...
const checkResponseStatus = (res) => {
  ...
  getClient = function getClient(accessToken, requestOptions = {}) {
    ...
    return Promise.resolve({
      ...
      apis: {
        appliances: {
          get_home_appliances: () => makeApiRequest("GET", "api/homeappliances", accessToken, null, requestOptions),
        },
        status: {
          get_status: (params) =>
            makeApiRequest("GET", `api/homeappliances/${params.haId}/status`, accessToken, null, requestOptions),
        },
        settings: {
          get_settings: (params) =>
            makeApiRequest("GET", `api/homeappliances/${params.haId}/settings`, accessToken, null, requestOptions),
        },
  ...
  refreshToken = (clientId, clientSecret, refreshTokenValue, requestOptions = {}) =>
    new Promise((resolve, reject) => {
      ...
      fetchWithTimeout(
        `${apiBaseUrl()}security/oauth/token`,
        {
          method: "POST",
...
class HomeConnect extends EventEmitter {
  constructor(clientId, clientSecret, refreshToken, options = {}) {
    super();
    // { debug, info, warn, error } - the helper passes its log so level filtering applies here too.
    this.log = options.logger || logToConsole;
    if (esLoadError) {
      this.log.error(`EventSource polyfill not available. Install 'eventsource'. ${esLoadError}`);
    }
  ...
  createEventSource(url) {
    ...
    const authFetch = (input, init = {}) => {
      const headers = new HeadersCtor(init?.headers ? init.headers : undefined);
  ...
  async init(options) {
    isSimulated =
      typeof options !== "undefined" && "isSimulated" in options && typeof options.isSimulated === "boolean"
        ? options.isSimulated
        : false;

    // Refresh tokens
    if (this.tokens.refresh_token) {
      this.tokens = await utils.refreshToken(this.clientId, this.clientSecret, this.tokens.refresh_token, {
  ...
  attachGlobalEventSourceMonitors() {
    ...
    if (!this._globalEventOpenListener) {
      this._globalEventOpenListener = () => {
        ...
        this.log.debug("EventSource open for global events");
      };
    }
  ...
  attachDeviceEventSourceMonitors(haid) {
    if (!haid || !this.eventSources?.[haid]) {
      return;
    }
    ...
    if (!monitorState.openListener) {
      monitorState.openListener = () => {
        ...
        this.log.debug(`EventSource open for ${haid}`);
      };
    }
  ...
  detachDeviceEventSourceMonitors(haid, source) {
    ...
    const monitorState = this._deviceEventMonitors[haid];
    const target = source || this.eventSources?.[haid];
  ...
  nextTokenRefreshDelaySeconds(error) {
    ...
    const grown = TOKEN_REFRESH_BASE_DELAY_S * 2 ** (attempt - 1);
  ...
  async refreshTokens() {
    ...
    try {
      ...
      // The open SSE streams stay as they are: every (re)connect reads the
      // current token through authFetch. Closing and reopening them here cost one
      // request per stream on each refresh and a gap in which events were lost.
      ...
      this.log.error(`Could not refresh tokens: ${error.message}`);
      this.log.error(`Retrying in ${timeToNextTokenRefresh}s (consecutive failures: ${this._tokenRefreshFailures})`);
    }
    this.tokenRefreshTimeout = setTimeout(() => this.refreshTokens(), timeToNextTokenRefresh * 1000);
  }
  ...
  closeEventSources(options = {}) {
    ...
    if (config.devices && this.eventSources) {
      for (const haid of Object.keys(this.eventSources)) {
        try {
          ...
          this.log.debug(`Closed device EventSource for ${haid}`);
        } catch (err) {
          this.log.warn(`Failed to close EventSource for ${haid}:`, err?.message ? err.message : err);
        }
      }
      this.eventSources = {};
      this.eventListeners = {};
    ...
    if (config.global && this.eventSource) {
      try {
        ...
        this.log.debug("Closed global EventSource");
      } catch (err) {
        this.log.warn("Failed to close global EventSource:", err?.message ? err.message : err);
      }
      this.eventSource = null;
      this.eventListener = new Map();
      this._globalEventMonitorAttached = false;
  ...
  nextEventSourceRetryDelayMs(label, err, { isAuthError, isRateLimited }) {
    ...
    const base =
      isAuthError || isRateLimited ? this.eventSourceRetryConfig.authDelayMs : this.eventSourceRetryConfig.baseDelayMs;
    const grown = base * 2 ** Math.min(state.attempts - 1, 10);
  ...
  handleEventSourceError(sourceLabel, err) {
    ...
    this.log.error(`EventSource error (${sourceLabel})${code ? ` [${code}]` : ""}: ${message}`);
    ...
    this.log.error(`Re-subscribing ${sourceLabel} in ${delay}ms due to event source error`);
    this.scheduleEventSourceRecreate(delay, sourceLabel);
  }

  recoverFromAuthError(delayMs, sourceLabel) {
    if (this._authRecoveryInFlight) {
      this.log.debug("Auth recovery already in progress - skipping additional trigger");
      return;
    }

    const fallback = () => {
      this.log.error(`Auth recovery failed - will retry ${sourceLabel} event source recreate in ${delayMs}ms`);
    ...
    this.refreshTokens()
      .then(() => {
        // Only the channel that failed was closed; the others keep running and
        // pick up the new token on their next connect.
        this.scheduleEventSourceRecreate(0, sourceLabel);
      })
      .catch((error) => {
        this.log.error("Token refresh during SSE auth recovery failed:", error?.message ? error.message : error);
        fallback();
      })
      .finally(() => {
        this._authRecoveryInFlight = false;
    ...
    if (label === "global") {
      if (this.eventSource && typeof this.eventSource.close === "function") {
        try {
          this.detachGlobalEventSourceMonitors();
          this.eventSource.close();
          this.log.debug("Closed global EventSource due to error");
        } catch (err) {
          this.log.warn("Failed to close global EventSource on error:", err?.message ? err.message : err);
        }
      }
      this.eventSource = null;
      this._globalEventMonitorAttached = false;
    ...
    if (label.startsWith("device:")) {
      ...
      if (source && typeof source.close === "function") {
        try {
          ...
          source.close();
          this.log.debug(`Closed device EventSource for ${haid} due to error`);
        } catch (err) {
          this.log.warn(`Failed to close device EventSource for ${haid} on error:`, err?.message ? err.message : err);
        }
      }
  ...
  scheduleEventSourceRecreate(delayMs, sourceLabel) {
    ...
    state.timer = setTimeout(() => {
      ...
      try {
        ...
        this.log.error(`Failed to recreate event source ${sourceLabel} after backoff:`, err);
      }
    }, jitteredDelay);
  }
  ...
  recreateEventSourceByLabel(label) {
    ...
    const listeners = this.eventListeners?.[haid];
    if (!listeners || listeners.size === 0) {
      return;
    }
  ...
  applyEventToDevice(device, event) {
    ...
    switch (key) {
      ...
      case "BSH.Common.Status.RemainingProgramTime":
        ...
        try {
          ...
          if (Number.isFinite(rem) && rem > 0) {
            const staleFinishedState = device.OperationState === "BSH.Common.EnumType.OperationState.Finished";
            const staleCompletedProgress =
              Number.isFinite(Number(device.ProgramProgress)) && Number(device.ProgramProgress) >= 100;
            ...
            noteRemainingSeen(device, rem, now);
          }
      ...
      case "BSH.Common.Status.OperationState": {
        const stateValue =
          typeof value === "string" ? value : value && typeof value.value === "string" ? value.value : value;
        const previousState = device.OperationState;
        device.OperationState = stateValue;
        trackDelayedStart(device, previousState, now);
