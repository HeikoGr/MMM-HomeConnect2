...
// Home Connect identifies every observable by exactly one fully qualified key, and
// these four are program options, so they arrive as `BSH.Common.Option.*` - there
// is no camelCase or snake_case spelling and no `BSH.Common.Status.*` twin for
// them (Status carries appliance state: OperationState, DoorState, ...). Two keys
// per value can nevertheless end up on a device object: applyEventToDevice() stores
// every event under its raw key and additionally mirrors it to the friendly name.
//
// Both spellings therefore have to be read - and cleared. Removing only the
// friendly name lets the raw key resurface through the parsers below, which is how
// a finished cycle used to keep a full progress bar alive.
const REMAINING_TIME_KEYS = Object.freeze([
  "RemainingProgramTime",
  "BSH.Common.Option.RemainingProgramTime"
]);

const PROGRESS_KEYS = Object.freeze(["ProgramProgress", "BSH.Common.Option.ProgramProgress"]);

const ESTIMATED_TOTAL_TIME_KEYS = Object.freeze([
  "EstimatedTotalProgramTime",
  "BSH.Common.Option.EstimatedTotalProgramTime"
]);

const ESTIMATED_DURATION_FLAG_KEYS = Object.freeze([
...
const OBSERVED_RUNTIME_KEYS = Object.freeze([
  "_initialRemaining",
  "_remainingObservedAt",
  "_lastRemainingSeenAt"
]);
...
// Removes a whole alias group, so a cleared value cannot come back through the
// raw key the API also stored.
...
function parseEstimatedTotalSeconds(device) {
  ...
  return findFirstParsed(
    readDeviceValues(device, ESTIMATED_TOTAL_TIME_KEYS),
    parseDurationSeconds,
    null
  );
}
...
function shouldDisplayDevice(device, config = {}) {
  ...
  if (
    config.showDeviceIfInfoIsAvailable &&
