...
function getDeviceId(device) {
  return device?.haId || device?.haid || device?.id || null;
}
...
function deviceAppearsActive(device) {
  ...
  const operationState = extractValueByType(device.OperationState, "string");
...
if (typeof window !== "undefined") {
  (function () {
    ...
    window.HomeConnectDeviceUtils.extractValueByType = exportsObj.extractValueByType;
    window.HomeConnectDeviceUtils.findFirstParsed = exportsObj.findFirstParsed;
    window.HomeConnectDeviceUtils.getDeviceId = exportsObj.getDeviceId;
  })();
}
