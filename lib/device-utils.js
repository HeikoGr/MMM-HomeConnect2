...
function hasInformativeState(device) {
  ...
  const selectedProgramOnly =
    device.ActiveProgramSource === "selected" && !deviceAppearsActive(device);
  ...
  return Boolean(
    (!selectedProgramOnly && device.ActiveProgramName) ||
    (!selectedProgramOnly && device.ActiveProgramPhase) ||
    (!selectedProgramOnly &&
