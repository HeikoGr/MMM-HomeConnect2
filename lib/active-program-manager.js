...
class ActiveProgramManager {
    ...
    schedule(devices, instanceId) {
        devices.forEach(device => {
            ...
            if (next > this.maxRetries) {
                this.logger.debug && this.logger.debug(`ActiveProgramManager: max retries reached for ${device.name}`);
                this.timers.delete(device.haId);
                return;
            }
            if (current && current.timeoutId) {
                this.logger.debug && this.logger.debug(`ActiveProgramManager: retry already scheduled for ${device.name}`);
                return;
            }
            this.logger.info && this.logger.info(`ActiveProgramManager: scheduling retry for ${device.name} in ${this.retryDelayMs}ms (attempt ${next}/${this.maxRetries})`);
    ...
    async _runRetry(device, instanceId, attempt) {
        ...
        try {
            this.logger.debug && this.logger.debug(`ActiveProgramManager: executing retry for ${device.name} (attempt ${attempt})`);
            const result = await this.fetchFn(device.haId, device.name);
            this.logger.debug && this.logger.debug(`ActiveProgramManager: retry result for ${device.name}`, { success: result.success, error: result.error || null });
            if (result.success && result.data) {
                // apply and broadcast via callback
                const payload = { [result.haId]: { name: device.name, program: result.data } };
            ...
            if (attempt < this.maxRetries && device && typeof device !== 'undefined') {
            ...
            this.logger.error && this.logger.error(`ActiveProgramManager: retry fetch failed for ${device.name}:`, err);
            this.clear(device.haId);
        }
    }
}
