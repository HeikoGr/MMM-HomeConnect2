...
class ActiveProgramManager {
  constructor(opts = {}) {
    ...
    this.isDeviceActiveFn = typeof opts.isDeviceActiveFn === "function" ? opts.isDeviceActiveFn : null;
  ...
  isRetryableProgramFetchFailure(result) {
    ...
    if (errorText.includes("timeout") || errorText.includes("etimedout") || errorText.includes("temporar")) {
      return true;
    }
  ...
  schedule(devices, instanceId) {
    devices.forEach((device) => {
      ...
      const timeoutId = setTimeout(() => this._runRetry(device, instanceId, next), this.retryDelayMs);
  ...
  async _runRetry(device, instanceId, attempt) {
    if (!device) return;
    // Guard: skip if the appliance went idle before the timer fired.
    if (this.isDeviceActiveFn && !this.isDeviceActiveFn(device.haId)) {
      this.logger.debug(`ActiveProgramManager: device ${device.name} no longer active - aborting retry`);
    ...
    try {
      this.logger.debug(`ActiveProgramManager: executing retry for ${device.name} (attempt ${attempt})`);
      ...
      if (result.success && result.data) {
        // The callback applies the result to the device and broadcasts it.
        const payload = {
          [result.haId]: { name: device.name, program: result.data },
        };
        this.broadcastFn(payload, instanceId, result);
        // Retries are only scheduled for running appliances. Answering with the
        // selected program means /programs/active was not ready yet (right after
        // the start) - ask again instead of showing "selected" until the next snapshot.
        if (result.source === "selected" && attempt < this.maxRetries) {
          // Re-check device is still active before scheduling another round.
          if (this.isDeviceActiveFn && !this.isDeviceActiveFn(device.haId)) {
            this.logger.debug(`ActiveProgramManager: device ${device.name} went idle - not retrying selected program`);
            this.clear(device.haId);
            return;
          }
      ...
      if (retryableFailure && attempt < this.maxRetries && device) {
