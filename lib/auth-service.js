...
class AuthService {
  ...
  setConfig(config) {
    ...
    if (this.setModuleLogLevel) {
      this.setModuleLogLevel(this.config?.logLevel || this.config?.loglevel || "none");
    }
  }

  async initiateDeviceFlow(clientId) {
    try {
      // eslint-disable-next-line n/no-unsupported-features/node-builtins -- fetch is stable in Node 20.6+
  ...
  async pollForToken(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.logger("info", `Starting token polling with ${currentInterval}s interval...`);
      const poll = async () => {
        attempts++;
        ...
        try {
          ...
          this.logger("error", "Network error during token polling:", fetchError);
          setTimeout(poll, currentInterval * 1000);
        }
      };
      setTimeout(poll, currentInterval * 1000);
  ...
  async headlessAuth(sendNotification) {
    ...
    try {
      ...
      this.logger("info", "Open the following URL on any device:", deviceAuth.verification_uri);
      this.logger("info", "User code:", deviceAuth.user_code);

      const completeLink =
        deviceAuth.verification_uri_complete ||
        `${deviceAuth.verification_uri}?user_code=${deviceAuth.user_code}`;
      this.logger("info", "Scan the QR code with your phone to open the verification link");
      ...
      this.logger("info", `Code expires in: ${Math.floor(deviceAuth.expires_in / 60)} minutes`);
      this.logger("info", `Polling interval: ${deviceAuth.interval || 5} seconds`);
  ...
  readRefreshTokenFromFile() {
    ...
    try {
      const token = fs.readFileSync("./modules/MMM-HomeConnect2/refresh_token.json", "utf8").trim();
  ...
  initiateAuthFlow() {
    ...
    if (!this.globalSession.isAuthenticating && !this.globalSession.refreshToken) {
