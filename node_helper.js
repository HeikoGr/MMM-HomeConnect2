...
async function headlessAuth(clientId, clientSecret, sendNotification) {
  ...
  try {
    ...
    try {
      ...
      verificationQrSvg = await QRCode.toString(
        completeLink,
        {
          "type": "svg",
...
module.exports = NodeHelper.create({
  ...
  handleConfigNotification(payload) {
    ...
    if (!this.configReceived) {
      ...
      if (this.config && typeof this.config === "object") {
        ...
        if (!("useHeadlessAuth" in this.config) && "use_headless_auth" in this.config) {
  ...
  readRefreshTokenFromFile() {
    ...
    try {
      ...
      console.log(
        "⚠️ Could not read refresh token file:",
        error.message
      );
      return null;
  ...
  handleHeadlessAuthError(error) {
    ...
    Log.log("❌ Max initialization attempts reached - aborting headless authentication");
    this.broadcastToAllClients(
      "INIT_STATUS",
      {
        "status": "auth_failed",
    ...
    try {
      ...
      const _self = this,
        ...
        tokens = await headlessAuth(
          ...
          (notification, payload) => _self.broadcastToAllClients(
            notification,
            payload
          )
        );
  ...
  handleHomeConnectInitError(error) {
    ...
    console.error(
      "❌ HomeConnect initialization failed:",
      error
    );
    globalSession.isAuthenticating = false;
  ...
  fetchDeviceSettings(device) {
    ...
    this.hc.
      ...
      then((settingsResult) => {
        ...
        `❌ Settings error for ${device.name}:`,
        error
      ));
  ...
  parseEvent(event, device) {
    ...
    const eventHandlers = {
      "BSH.Common.Option.RemainingProgramTime": () => this.parseRemainingProgramTime(
        device,
        event.value
      ),
      "BSH.Common.Option.ProgramProgress": () => this.parseProgramProgress(
        device,
        event.value
      ),
      "BSH.Common.Status.OperationState": () => this.parseOperationState(
        device,
        event.value
      ),
      "Cooking.Common.Setting.Lighting": () => this.parseLighting(
        device,
        event.value
      ),
      "BSH.Common.Setting.PowerState": () => this.parsePowerState(
        device,
        event.value
      ),
      "BSH.Common.Status.DoorState": () => this.parseDoorState(
        device,
        event.value
      )
    },
