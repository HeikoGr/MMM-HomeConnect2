...
- `lib/` (except `lib/mmm-shared/`), `translations/`
- `node_helper.js`, `MMM-HomeConnect2.js`, `MMM-HomeConnect2.css`
...
`auth`, `device`, `program`, `api`, `config`, `i18n`, `deps`, `devcontainer`

- `auth` - `lib/auth-service.js`, the OAuth2 device-flow login/token handling
- `device` - `lib/device-service.js`, `lib/device-utils.js`, appliance discovery and state
- `program` - `lib/program-service.js`, `lib/active-program-manager.js`, active/selected program
  polling
- `api` - `lib/homeconnect-api.js`, the BSH Home Connect REST/SSE client
- `config` - module configuration handling and defaults, `config/` templates
- `i18n` - `translations/`
...
- Good: `fix(auth): retry the device-flow poll after a transient 5xx`
- Bad: `fix(auth): update auth-service.js`
- Bad: `fix(auth): various fixes and improvements`
...
```
feat(config)!: require an explicit clientId in module config

BREAKING CHANGE: the module no longer falls back to a bundled demo clientId. Set `clientId` in
your `config.js` before starting the module.
```

## Examples from this repository

```
fix(device): stop reporting a stale program when the SSE stream reconnects
feat(program): surface remaining program time in the module DOM
fix(api): back off from the Home Connect API after a rate-limit response
refactor(auth): extract token refresh into its own function
build(devcontainer): install the GitHub CLI
docs: correct the wiki links in README.md
```
